...
if(isset($_POST['signup'])){
    ...
    $sql = "INSERT INTO user (name, username, password,  role) 
       ...
       if (mysqli_query($conn, $sql)) {
        ...
        $update_sql = "UPDATE user SET student_id = $last_id WHERE id = $last_id";
        mysqli_query($conn, $update_sql);
        $message = "<div class='alert alert-success text-center' role='alert'>$name added successfully!!</div>";
    } else {
        $message = "<div class='alert alert-danger text-center' role='alert'>Error with adding a new student: " . mysqli_error($conn) . "</div>";
    }
} else {
